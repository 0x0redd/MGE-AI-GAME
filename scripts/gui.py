...
class Upgrader(CellsBar):
    def __init__(self, left, centery, game_instance=None):
        # Use game_instance if provided, otherwise fall back to global game
        game_ref = game_instance if game_instance is not None else game
        super().__init__(game_ref.asset.upgrade_bar, left, centery, 10)
        self.colors = ['yellow', 'gold', 'orange', 'darkorange', 'tomato', 'orangered', 'red', 'crimson', 'firebrick', 'darkred']
        self.cells = []
        for i in range(self.max):
            self.cells.append(c.swap_color(game_ref.asset.upgrade_cell.copy(), 'white', self.colors[i]))
...
class Container(CellsBar):
    def __init__(self, left, centery, game_instance=None):
        # Use game_instance if provided, otherwise fall back to global game
        game_ref = game_instance if game_instance is not None else game
        super().__init__(game_ref.asset.container_bar, left, centery, 3)
        self.cell = c.swap_color(game_ref.asset.container_cell.copy(), 'white', 'cyan')
...
class PopUp():
    ...
    time  = 2000
    width = 280
    def __init__(self, text, game_instance=None):
        game_ref = game_instance if game_instance is not None else game
        ...
        self.surface.blit(game_ref.asset.icon_success, (5, 5))
...
class EffectsHud():
    def __init__(self, game_instance=None):
        ...
        self.freeze = game_ref.gd.freezes
        self.repulsion = game_ref.gd.repulsions
    ...
    def rect(self):
        if self._rect:
            ...
            self._rect = (self._game_ref.player.rect.left - 37, self._game_ref.player.rect.y - 37, 124, 124)
            return self._rect
    ...
    def update(self, dt, surf):
        ...
        if hasattr(self._game_ref, 'power_timer') and self._game_ref.power_timer > 0:
    ...
    def render_timer(self, surf):
        rect = (self._game_ref.player.rect.left - 30, self._game_ref.player.rect.top - 30, 110, 110)
        end = c.PI_2 - max(1 - self._game_ref.power_timer / c.POWER_DELAY, 0) * c.TWO_PI
