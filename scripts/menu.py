...
class SpaceshipPage(BasePage):
    def __init__(self, game):
        ...
        self.upgrader = Upgrader(100 + text_size[0], 248 + text_size[1] / 2, self.g)
        ...
        self.freeze = Container(100 + text_size[0], 323 + text_size[1] / 2 + sep, self.g)
        ...
        self.repulsion = Container(126 + text_size[0], 398 + text_size[1] / 2 + sep, self.g)
...
class GameOverMenu():
    ...
    def _slot_replay(self):
        ...
        self.g.setup_game()
        # Start the game directly without going through menu state
        self.g.menu_active = False
        self.g.game_start_time = time()
        self.g.gd.incr_game_nb()
        pygame.mouse.set_visible(False)
        # Show camera window for hand tracking during gameplay
        try:
            self.g.gesture_controller.show_camera_window()
        except:
            pass  # In case there's an issue with camera
        # Clear and spawn first asteroid
        self.g.asteroids.clear()
        self.g.spawn_asteroid()
        if self.g.asteroids:  # Make sure asteroid was spawned
            self.g.first_asteroid = self.g.asteroids[0]
        # Set player position
        self.g.player.rect.center = c.MOUSE_POS
...
class Menu():
    ...
    def set_popup(self, message):
        self.popup = gui.PopUp(message, self)
