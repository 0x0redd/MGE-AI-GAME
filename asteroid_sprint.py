...
import scripts.brand_config as brand


class Game():
    def __init__(self):
        self.window = ShaderWindow(c.WIN_SIZE, brand.GAME_TITLE, 'shaders/')
        ...
        self.circuit_pattern = CircuitPattern(c.WIN_SIZE[0], c.WIN_SIZE[1])
        self.geometric_pattern = GeometricPattern(c.WIN_SIZE[0], c.WIN_SIZE[1])
        ...
        self.setup_shaders()
        self.hud = gui.EffectsHud(self)
        
    def setup_ui(self):
        self.font = brand.FONT_PRIMARY
        c.fp = self.font
        self.ui_surf = pygame.Surface(c.WIN_SIZE, pygame.SRCALPHA)
        self.time_font = pygame.font.Font(self.font, brand.FONT_SIZE_MEDIUM)
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if event.key == pygame.K_c:
                    ...
                    self.gesture_controller.toggle_camera_feed()
                elif event.key == pygame.K_h:
                    # Show/hide help with H key
                    self.show_help = not getattr(self, 'show_help', False)
        
        # Get hand position from gesture controller
        c.MOUSE_POS = self.gesture_controller.get_hand_position()
    ...
    def draw_help_overlay(self, surface):
        """Draw help overlay with controls information"""
        # Semi-transparent background
        overlay = pygame.Surface(c.WIN_SIZE, pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 180))
        surface.blit(overlay, (0, 0))
        
        # Help content
        font_large = pygame.font.Font(brand.FONT_PRIMARY, brand.FONT_SIZE_LARGE)
        font_medium = pygame.font.Font(brand.FONT_PRIMARY, brand.FONT_SIZE_MEDIUM)
        font_small = pygame.font.Font(brand.FONT_PRIMARY, brand.FONT_SIZE_SMALL)
        
        # Title
        title = font_large.render("Controls", True, brand.BRAND_GREEN)
        title_rect = title.get_rect(center=(c.WIN_SIZE[0]//2, 80))
        surface.blit(title, title_rect)
        
        # Hand Gestures
        gesture_title = font_medium.render("Hand Gestures:", True, brand.BRAND_WHITE)
        surface.blit(gesture_title, (50, 140))
        
        gestures = [
            "Point index finger → Move spaceship",
            "Make a fist → Repulse ability",
            "Peace sign (✌️) → Shield activation"
        ]
        
        for i, gesture in enumerate(gestures):
            text = font_small.render(gesture, True, brand.BRAND_LIGHT_GREY)
            surface.blit(text, (70, 170 + i * 25))
        
        # Keyboard Controls
        keyboard_title = font_medium.render("Keyboard Controls:", True, brand.BRAND_WHITE)
        surface.blit(keyboard_title, (50, 260))
        ...
        for i, key in enumerate(keys):
            text = font_small.render(key, True, brand.BRAND_LIGHT_GREY)
            surface.blit(text, (70, 290 + i * 20))
        
        # Close instruction
        close_text = font_small.render("Press H to close", True, brand.BRAND_GREEN)
        close_rect = close_text.get_rect(center=(c.WIN_SIZE[0]//2, c.WIN_SIZE[1] - 50))
    ...
    def start_game(self):
        ...
        # Show camera window for hand tracking during gameplay
        self.gesture_controller.show_camera_window()
    ...
    def end_game(self):
        ...
        # Hide camera window when game ends
        self.gesture_controller.hide_camera_window()
    ...
    def reset(self):
        self.setup_game()
        self.menu_active = True
        # Hide camera window when returning to menu
        self.gesture_controller.hide_camera_window()
    ...
    def run(self):
        ...
        while True:
            ...
            # reset window with brand colors
            self.display.fill(brand.GAME_BACKGROUND)
            
            # Update and draw background patterns
            self.circuit_pattern.update(self.dt / 1000.0)
            self.geometric_pattern.update(self.dt / 1000.0)
            
            # Initialize offset for background patterns (will be updated later for screen shake)
            bg_offset = 0
            
            # Draw geometric patterns first (background)
            self.geometric_pattern.draw(self.display, (bg_offset, bg_offset))
            
            # Draw circuit patterns
            self.circuit_pattern.draw(self.display, (bg_offset, bg_offset))
            ...
            # Camera feed is displayed in a separate window by GestureController
            
            # Draw help overlay
            if getattr(self, 'show_help', False):
                self.draw_help_overlay(self.ui_surf)
